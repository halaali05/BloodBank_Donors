...
exports.updateFcmToken = onCall(async (request) => {
  try {
    ...
    // Ensure a token belongs to only one user at a time.
    // This prevents cross-account notifications on the same device.
    ...
    if (!duplicateTokenUsers.empty) {
      ...
      if (cleanedCount > 0) {
        ...
        console.log(
            `[updateFcmToken] Removed duplicate token from ${cleanedCount} user(s) before assigning to ${uid}`,
        );
      }
    }
